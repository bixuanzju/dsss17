...
(* It comes with a proof that every element in the interval can be generated by
some seed *)
...
Sample (genListSized 3 (choose(0,3))).
...
Definition genColorOption : G (option color) :=
  freq [(2, ret None); (9, ret (Some Red)); (3, ret (Some Green)); (3, ret (Some Blue));(3, ret (Some Yellow))].
...
Local Open Scope string.
Instance show_term_tree {A} `{Show A} : Show (TernaryTree A) :=
  {
    show :=
       let fix aux t :=
         match t with
           | TLeaf => "TLeaf"
           | TNode x l m r =>
               "TNode (" ++ show x ++ ") (" ++ aux l ++ ") (" ++ aux m ++ ") (" ++ aux r ++ ")"
         end
       in aux
  }.
...
Definition liftM4 {T U V W S} `{Monad G} (f : T -> U -> V -> W -> S) (x : G T) (y : G U) (z : G V) (g : G W): G S :=
  x' <- x ;;
  y' <- y ;;
  z' <- z ;;
  g' <- g ;;
...
Fixpoint genTernTreeSized {A} (sz : nat) (g : G A) : G (TernaryTree A) :=
  match sz with
    ...
    | S sz' =>
        freq [ (1,  ret TLeaf) ;
               (sz, liftM4 TNode g (genTernTreeSized sz' g) (genTernTreeSized sz' g) (genTernTreeSized sz' g))
             ]
  end.
...
(* Finite maps as lists of pairs *)

Definition Map := list (nat * nat).

Definition empty : Map := nil.

Function add (k : nat) (x : nat) (s : Map) {struct s} : Map :=
  match s with
   | nil => (k,x) :: nil
   | (k',y) :: l => if Nat.eqb k k' then (k,x)::l else (k',y)::add k x l
  end.

Function find (k:nat) (s: Map) {struct s} : option nat :=
  match s with
   | nil => None
   | (k',x)::s' => if Nat.eqb k k' then Some x else find k s'
  end.
...
Fixpoint gen_bst (sz : nat) (low high : nat) : G (Tree nat) :=
  match sz with
    | O => ret Leaf
    | S sz' =>
        freq [ (1,  ret Leaf) ;
               (sz, (x <- choose (low+1, high-1) ;; l <- gen_bst sz' low high ;; ret (insertBST' x l)))
             ]
  end.

(* let's test tress within bound (0,10) *)

Definition insertBST_test :=
  forAllShrink ( l <- gen_bst 10 0 10 ;; x <- choose (1, 9) ;; ret (l, x)) shrink
